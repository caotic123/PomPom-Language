(* Final mu-application typing proof from only the specialized Pi-with-sort
   codomain property; full Pi injectivity is unnecessary. *)
...
Lemma pi_cod_sort_inv_intro_specialized_parent :
    conv_pi_sort_codomain_compatible_parent ->
    forall A0 B0 j,
      conv B0 (TSort j) -> pi_cod_sort_inv_glm (TPi A0 B0).
Proof.
  intros PICOMP A0 B0 j HB A B Hpi.
  exists j. apply (PICOMP A0 A B j).
  eapply cv_trans.
  - apply cv_pi; [apply cv_refl | apply cv_sym; exact HB].
  - exact Hpi.
Qed.

Lemma sub_pi_cod_sort_inv_specialized_parent :
    conv_pi_sort_codomain_compatible_parent ->
    forall G X Y,
      sub G X Y -> pi_cod_sort_inv_glm X -> pi_cod_sort_inv_glm Y.
Proof.
  intros PICOMP G X Y Hsub.
  ...
  - destruct (pi_cod_sort_inv_pi_elim_glm _ _ Hinv) as [j Hj].
    destruct (glm_sub_sort_source_conv_glm _ B B' Hsub2 j Hj)
      as [j' Hj'].
    eapply pi_cod_sort_inv_intro_specialized_parent; eassumption.
...
Lemma sub_pisort0_cod_sort_specialized_parent :
    conv_pi_sort_codomain_compatible_parent ->
    forall G IT A B,
      sub G (TPi IT (TSort 0)) (TPi A B) ->
      exists j, conv B (TSort j).
Proof.
  intros PICOMP G IT A B Hsub.
  eapply pi_cod_sort_inv_pi_elim_glm.
  eapply sub_pi_cod_sort_inv_specialized_parent;
    [exact PICOMP | exact Hsub |].
  eapply pi_cod_sort_inv_intro_specialized_parent;
    [exact PICOMP | apply cv_refl].
Qed.

Lemma ao_chk_mui_sort_specialized_parent :
    conv_pi_sort_codomain_compatible_parent ->
...
Proof.
  intros PICOMP R a A B k T U h Hk Hf Ha Hsub Hc Hw.
  ...
  { destruct Hrel as [Hrel | Hrel].
    - exists 0. apply subst0_sort_target_glm.
      exact (PICOMP IT A B 0 Hrel).
    - destruct (sub_pisort0_cod_sort_specialized_parent
        PICOMP [] IT A B Hrel) as [j Hj].
      exists j. apply subst0_sort_target_glm. exact Hj. }
  eapply ao_chk_sort_glm; eassumption.
Qed.

Lemma ao_chk_mus_sort_specialized_parent :
    conv_pi_sort_codomain_compatible_parent ->
    forall Sf a A B k T U h,
      ...
      conv T U -> whd U h -> h = HSort.
Proof.
  intros PICOMP Sf a A B k T U h Hk Hf Ha Hsub Hc Hw.
...
Theorem muapp_sort_from_pi_sort_parent :
    conv_pi_sort_codomain_compatible_parent ->
    forall f a T U h,
      check [] (TApp f a) T -> value (TApp f a) ->
      conv T U -> whd U h -> h = HSort.
Proof.
  intros PICOMP f a T U h Hchk Hv Hc Hw.
  ...
  - subst X.
    ...
    + rewrite Hf' in Hf.
      eapply ao_chk_mui_sort_specialized_parent; eassumption.
    + rewrite Hf' in Hf.
      eapply ao_chk_mus_sort_specialized_parent; eassumption.
Qed.

Corollary muapp_sort_from_phi_sort_endpoint_parent :
    phi_erase_sort_endpoint_reflect_parent ->
    forall f a T U h,
      check [] (TApp f a) T -> value (TApp f a) ->
      conv T U -> whd U h -> h = HSort.
Proof.
  intros REF. apply muapp_sort_from_pi_sort_parent.
  exact (conv_pi_sort_codomain_from_phi_direct_parent REF).
Qed.

Print Assumptions sub_pi_cod_sort_inv_specialized_parent.
Print Assumptions muapp_sort_from_pi_sort_parent.
Print Assumptions muapp_sort_from_phi_sort_endpoint_parent.
