...
Proof.
  ...
  all: intros.
  all: try (sim_clear_premises; sim_inv_mueq; sim_struct).
  all: try sim_beta.
  all: try sim_app.
  all: try sim_fst_pair.
  all: try sim_snd_pair.
  all: try sim_epi_nil.
  all: try sim_epi_cons.
  all: try sim_switch_zero.
  all: try sim_switch_succ.
  all: try sim_interp_var.
  all: try sim_interp_one.
  all: try sim_interp_prod.
  all: try sim_interp_pi.
  all: try sim_interp_sig.
  all: try sim_interp_choice.
  all: try sim_iall_var.
  all: try sim_iall_one.
  all: try sim_iall_prod.
  all: try sim_iall_pi.
  all: try sim_iall_sig.
  all: try sim_iall_choice.
  all: try sim_hyps_var.
  all: try sim_hyps_one.
  all: try sim_hyps_prod.
  all: try sim_hyps_pi.
  all: try sim_hyps_sig.
  all: try sim_hyps_choice.
  all: try sim_ind_red.
  all: try sim_case.
  all: try sim_case_red.
  all: try sim_pbs_nil.
  all: try sim_pbs_cons.
Qed.

(* ------------------------------------------------------------------ *)
(*  Corollaries: the named simulation squares                          *)
(* ------------------------------------------------------------------ *)

Corollary pstep_mueq_sim : conv_subst_compatible ->
  forall t u, pstep t u -> forall t', mueq t t' ->
    exists u', pstep t' u' /\ mueq u u'.
Proof.
  intros Hc t u H.
  exact (proj1 (pstep_mueq_sim_mut Hc) t u H).
Qed.

Corollary pbranches_mubeq_sim : conv_subst_compatible ->
  forall bs bs', pbranches bs bs' -> forall bs'', mubeq bs bs'' ->
    exists bs''', pbranches bs'' bs''' /\ mubeq bs' bs'''.
Proof.
  intros Hc bs bs' H.
  exact (proj2 (pstep_mueq_sim_mut Hc) bs bs' H).
Qed.

(* The square read against the mueq-symmetric side. *)
Corollary pstep_mueq_sim_sym : conv_subst_compatible ->
  forall t u, pstep t u -> forall t', mueq t' t ->
    exists u', pstep t' u' /\ mueq u' u.
Proof.
  intros Hc t u H t' Hm.
  destruct (pstep_mueq_sim Hc t u H t' (mueq_sym _ _ Hm)) as [u' [Hs Hm']].
  exists u'; split; [exact Hs | apply mueq_sym; exact Hm'].
Qed.
