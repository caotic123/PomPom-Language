...
(*  minimal language), not an algorithm and not a soundness proof — the §10   *)
(*  metatheory (progress, preservation, canonical forms, normalization,       *)
(*  consistency: no bottom out of thin air) is STATED in §8 as Conjectures.   *)
...
(* ========================================================================== *)
(*  8. Metatheory, STATED — §10: 'subject reduction and canonical forms for   *)
(*     the μˢ layer — the paper's claims for its fragment, restated for this  *)
(*     rendition; normalization and consistency under the predicative         *)
(*     universe policy'.  These are the sketch's stated proof obligations:    *)
(*     Conjectures here, with only their routine corollaries proved.          *)
(* ========================================================================== *)
...
(* -------------------------------------------------------------------------- *)
(*  Progress.  Paper (progress at signature types): 'If ⊢ M : {T Δ* :: Φ},    *)
(*  then either M is a value or there exists M2 such that M → M2.'  Stated    *)
(*  for every closed checked term of the calculus.                            *)
(* -------------------------------------------------------------------------- *)
(*  DISCHARGED RELATIVE TO A NAMED INTERFACE: Progress.v proves this exact   *)
(*  statement (progress_proved) by size-measure induction, standing on five  *)
(*  confluence-grade conversion facts (conv_whd, conv_enumt_inj, conv_pos,   *)
(*  spine_covered, muapp_sort) and canonical_forms_sig below — certified by  *)
(*  Print Assumptions there.                                                 *)
Conjecture progress : forall t A,
    check [] t A -> value t \/ exists t', step t t'.

(*  §10: 'normalization … under the predicative universe policy'.             *)
Conjecture normalization : forall t A,
    check [] t A -> exists v, eval t v /\ value v.

(*  Canonical forms at μˢ — §7 Operational reading: 'a closed M : μˢ S i      *)
(*  weak-head-normalizes to in (c, xs) with Member c (labels (S i)) and       *)
(*  xs : ⟦branches (S i) c⟧ (Carrier S).  This is what makes Sig-case's       *)
(*  coverage sound, and is a stated proof obligation.'                        *)
(*  The index premise is load-bearing, like the rest of the formation data:   *)
(*  eta makes conv (λx. 0 x) 0 a raw fact, so ch_expand re-types a valid      *)
(*  node at the ill-typed index λx. 0 x — where a signature that switches     *)
(*  on its index is permanently stuck and no spine is ever exposed            *)
(*  (kernel-checked counterexample to the statement without it).              *)
...
(* -------------------------------------------------------------------------- *)
(*  Preservation (subject reduction) — §10: 'subject reduction … for the μˢ   *)
(*  layer', and 'type preservation of the switchₖ, projection, iinduction,    *)
(*  and case computation rules inside ≡'; with Conversionφ default it also    *)
(*  covers Eq-φ, through against_sound below.                                 *)
(* -------------------------------------------------------------------------- *)
Conjecture preservation : forall Γ t t' A,
    check Γ t A -> step t t' -> check Γ t' A.

(*  Multi-step preservation follows by induction on ⇓.                        *)
...
(*  Typing is also conserved when the TYPE computes — this one is already     *)
(*  derivable from the system's own conversion rules.                         *)
...
(* -------------------------------------------------------------------------- *)
(*  Consistency — §10: '… and consistency under the predicative universe      *)
(*  policy': we do not derive bottom out of thin air.                         *)
(* -------------------------------------------------------------------------- *)

(* the polymorphic empty type  ⊥ := (X : Set₀) → X *)
Definition Bot : term := TPi (TSort 0) (TVar 0).

Conjecture consistency : forall t, ~ check [] t Bot.

(* the empty enumeration has no closed element *)
Conjecture consistency_enum : forall t, ~ check [] t (TEnumT TNilE).

(* Sig-formation is liberal — 'the refinement is then uninhabited there,      *)
(* which is harmless' (§7) — and the paper adds: 'there is no rule that       *)
(* introduces a signature type without constructor evidence'.  So an          *)
(* empty-spined refinement conjures no inhabitant:                            *)
Conjecture consistency_empty_sig : forall M Sf i A,
    check [] M (TApp (TMuS Sf) i) ->
    eval (labels (TApp Sf i)) (TLNil A) ->
    False.

(* Soundness of the AGAINST analog (promised in §4): on a WELL-TYPED payload  *)
(* description, a derived pruning really is an uninhabited payload, so Eq-φ   *)
(* never conjures a value and Conversionφ preserves consistency.  The typing  *)
(* premises (those of the ⟦_⟧ formation rule) scope the claim to where φ      *)
(* uses it — sph_drop prunes payloads of signatures that reach it through     *)
(* Sig-typed premises.  (An untyped statement was refuted while raw case      *)
(* selection was still free: a duplicate-label case evaluated both to a dead  *)
(* choice and to an inhabited code.  Raw selection is now first-match —       *)
(* st_case — but the typed scoping stays: soundness is owed only for          *)
(* well-typed descriptions.)                                                  *)
...
(* ========================================================================== *)
(*  9. What is deliberately not here (per the sketch)                         *)
(*                                                                            *)
...
(*   - §10's metatheory PROOFS: progress, preservation, canonical forms,      *)
(*     normalization, consistency, and against_sound are STATED in §8 as      *)
(*     Conjectures — the sketch's proof obligations — with only their         *)
(*     routine corollaries (multi-step and type-side preservation) proved.    *)
(*     Decidability of checking and agreement of the internal algebra         *)
(*     (progress/2.pom's decision pipeline) with the core spine checks        *)
(*     remain unstated here.  With Conversionφ default the bundle includes           *)
(*     against_sound, which is what lets Eqφ preserve typing and Sig-case     *)
(*     take Ψ = Φok through conversion.                                       *)
(* ========================================================================== *)
